...
Retail recommendation has long relied on transactional co-occurrence patterns and collaborative filtering. Association-rule mining made basket analysis practical at scale by deriving frequent item relationships with interpretable support and confidence metrics \cite{agrawal1994association}. Later recommender work expanded toward user--item preference modeling, including item-based collaborative filtering that improved scalability and reuse across large catalogs \cite{sarwar2001itembased}. RetailPulse stays closer to the association-rule tradition because the project goal is auditable cross-sell evidence from order baskets rather than personalized ranking under dense feedback assumptions.

\subsection{Customer Segmentation and Clustering}
Customer segmentation is commonly used to summarize behavioral heterogeneity before any personalized intervention is attempted. KMeans remains one of the most widely used baselines for this purpose because it is simple, scalable, and easy to explain \cite{macqueen1967kmeans}. More recent work continues to refine KMeans-based customer segmentation with improved feature engineering or optimization strategies \cite{li2021customersegmentation}. RetailPulse follows the descriptive segmentation lane: the goal is not to claim causal customer archetypes, but to create stable, warehouse-backed behavioral groups that can be inspected in downstream dashboards and reports.

\subsection{Lakehouse and Streaming Analytics Systems}
The system dimension of retail analytics has also evolved. Delta Lake introduced ACID table storage and schema-aware management over cloud object stores \cite{armbrust2020deltalake}, while the lakehouse model argued for unifying data warehousing and advanced analytics within a single platform \cite{zaharia2021lakehouse}. On the streaming side, Structured Streaming provided a declarative model for integrating incremental processing with batch-oriented analytics workflows \cite{armbrust2018structured}. RetailPulse builds on these ideas in a constrained educational setting: it uses a lakehouse-style Delta pipeline, Structured Streaming-compatible validation logic, Databricks Asset Bundles for orchestration \cite{databricks_bundles}, and Databricks AI/BI dashboards for reviewer-facing presentation \cite{databricks_dashboards}. The gap addressed by this paper is therefore an integration gap rather than an algorithmic gap: the need for a single, defensible framework that connects descriptive analytics, recommendation evidence, validation, and reporting under explicit scope limits.
...
The sampled CSV files are uploaded into a Unity Catalog raw volume and ingested through explicit schemas. This step matters because it prevents hidden type drift and establishes a stable contract for downstream Delta tables. The ingestion and rebuild flow is packaged as a sequential Databricks Asset Bundle job, which turns the notebook chain into a reproducible execution unit rather than a manual classroom demo \cite{databricks_bundles}.
...
The descriptive analytics layer is materialized rather than left implicit in notebook outputs. RetailPulse runs a set of OLAP-oriented queries using Spark SQL \cite{spark_sql}: a cube over department and day-of-week demand, a rollup over day-of-week and hour-of-day order behavior, and a basket-analysis aggregation by daypart and department. The resulting tables are persisted as report artifacts so the dashboard and the report pack can read them without recomputing logic.
...
The descriptive layer converts the gold facts into reusable analytical summaries. Figure~\ref{fig:orders_by_day} illustrates the day-of-week rollup that combines order counts with average basket size. The figure is important less as a one-off screenshot than as evidence that the OLAP lane is backed by persisted warehouse outputs. This matters because the same rollup feeds both the dashboard and the report pack.

More broadly, the OLAP notebook generates three reusable views of order behavior: category demand through a cube aggregation, temporal order patterns through a rollup, and daypart-by-department basket analysis. Because the source dataset does not contain absolute dates, the findings are intentionally phrased in terms of relative day and hour behavior rather than month-over-month or seasonal trends.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{10_orders_by_day.png}
\caption{Order-behavior evidence from the persisted day-of-week rollup. The chart summarizes order-count and average-basket behavior by \texttt{order\_dow} rather than by unavailable calendar dates.}
...
\caption{Silhouette-score evidence for the tested KMeans configurations. Making the tested \(k\) values visible improves the interpretability of the final segmentation choice.}
\label{fig:k_selection}
\end{figure}

\subsection{Validation, Optimization, and Exploratory Modeling}
The strongest correctness evidence in RetailPulse comes from the validation lane. Table~\ref{tab:stream_validation} shows that the replay-style stream checked 158 order-slot groups and produced zero mismatches. Table~\ref{tab:olap_validation} complements this result by confirming that base-grain cube totals matched the equivalent direct fact-table aggregation. Together, these tables establish that the descriptive and streaming layers are computationally consistent for the validated release.
...
Table~\ref{tab:exploratory_metrics} therefore remains secondary to the validation and recommendation tables. The classifier outperformed a majority baseline, and the regression outperformed a mean baseline, but those gains do not override the current feature-design weaknesses discussed in Section~V.

\begin{table}[!t]
\caption{Replay-stream validation summary.}
\label{tab:stream_validation}
\centering
\footnotesize
...
Third, the sampling strategy improves reproducibility at the cost of external validity. The deterministic user-based subsample is appropriate for stable reruns and classroom-scale experimentation, but it is still a subset of the original dataset. Any observed behavioral pattern should therefore be interpreted as evidence from the validated sample rather than as a universal statement about all online grocery shopping.
